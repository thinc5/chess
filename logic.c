    ...
    for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++)
        {
            ...
            size_t num_possible_moves = get_possible_moves_for_piece(
                board, i, possible_moves, move_count, 0);
 ...
 */
size_t
is_game_stalemate(Board board, PlayerColour player, size_t move_count)
{
    size_t possible = 0;
    for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++)
        {
            if (board[i].colour != player) { continue; }
            PossibleMove possible_moves[MAX_POSSIBLE_MOVES] = { 0 };
            size_t num_possible_moves = get_possible_moves_for_piece(
                board, i, possible_moves, move_count, 0);
 ...
 */
size_t
is_game_over_for_player(Board board, Board next_board, PlayerColour player,
    ...
    for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++)
        {
            ...
            PossibleMove possible_moves[MAX_POSSIBLE_MOVES] = { 0 };
            size_t possible = get_possible_moves_for_piece(
                board, i, possible_moves, move_count, check);
            ...
            for (int m = 0; m < possible; m++)
                {
                    ...
                    if (process_movement(next_board, move_count, i,
                            possible_moves[m].target, check)
                        != MOVEMENT_ILLEGAL)
                        {
                            size_t checkmate = is_checkmate_for_player(
                                next_board, player, move_count, check);
                            // Reset the state.
