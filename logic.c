...
MovementType process_movement(Board board, size_t move_count, int selected, int dest, size_t check) {
    ...
    MovementType movement = PIECE_MOVEMENT_ALGORITHM[selected_piece->type](board, selected, dest, move_count, check);
...
size_t is_game_over_for_player(Board board, Board next_board, PlayerColour player, size_t move_count, size_t check) {
    ...
    for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {
        ...
        // Get possible moves.
        PossibleMove possible_moves[MAX_POSSIBLE_MOVES] = {0};
        size_t possible = get_possible_moves_for_piece(board, i, possible_moves, move_count, check);
        if (possible < 1) {
            continue;
        }
        ...
        for (int m = 0; m < possible; m++) {
            ...
            if (process_movement(next_board, move_count, i, possible_moves[m].target, check) != MOVEMENT_ILLEGAL) {
                size_t checkmate = is_checkmate_for_player(next_board, player, move_count, check);
                // Reset the state.
