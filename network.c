...
int host_server(const char *port) {
    ...
    getnameinfo((struct sockaddr*)&server_addr, sizeof(server_addr), readable_hostname, sizeof(readable_hostname), 0, 0, NI_NUMERICHOST);
    printf("Server listening... (fd: %d) on: %s:%d\n", socket_fd, readable_hostname, port_num);
...
int read_network_line(int fd, char *input_buffer) {
    ...
    while (bytes_read < INPUT_BUFFER_SIZE) {
        int read_chunk = read(fd, input_buffer + bytes_read, INPUT_BUFFER_SIZE - bytes_read);
        if (read_chunk <= 0) {
            break;
        }
