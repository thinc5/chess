...
host_server(const char *port)
{
    ...
    getnameinfo((struct sockaddr *) &server_addr, sizeof(server_addr),
        readable_hostname, sizeof(readable_hostname), 0, 0, NI_NUMERICHOST);
    ...
    int connection_fd
        = accept(socket_fd, (struct sockaddr *) &client_addr, &len);
    if (connection_fd < 0)
        {
...
int
connect_to_sever(const char *address, const char *port)
{
    int socket_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (socket_fd == -1)
        {
    ...
    if (connect(
            socket_fd, (struct sockaddr *) &server_addr, sizeof(server_addr))
        != 0)
        {
...
int
read_network_line(int fd, char *input_buffer)
{
    fd_set read_fds;
    FD_ZERO(&read_fds);
    FD_SET(fd, &read_fds);
    ...
    while (bytes_read < INPUT_BUFFER_SIZE)
        {
            int read_chunk = read(
                fd, input_buffer + bytes_read, INPUT_BUFFER_SIZE - bytes_read);
...
int
write_network_line(int fd, char *input_buffer, int size)
{
