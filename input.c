...
static inline bool valid_char_pairing(char x, char y) {
    return !(tolower(x) < 'a' || tolower(x) > 'a' + BOARD_SIZE || y < '0' || y > '0' + BOARD_SIZE);
}

/**
 * Pure textual validation with operation mode context, no real validation is performed here.
 * Returns intended command or COMMAND_INVALID if the command is not matched.
 */
