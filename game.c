...
void
clear_piece_selection(ChessGame *game)
{
...
void
clear_input_buffer(ChessGame *game)
{
...
void
init_chess_game(ChessGame *game)
{
...
void
toggle_player_turn(ChessGame *game)
{
    game->turn = (game->turn + 1) % NUM_PLAYER_COLOURS;
}

bool
move_piece(ChessGame *game, Command command)
{
    int move_dest
    ...
    if (is_checkmate_for_player(game->next_board,
            game->next_board[move_dest].colour, game->move_count, game->check))
        {
...
play_chess(ChessGame *game)
{
    ...
    while (true)
        {
            game->check = is_checkmate_for_player(
                game->board, game->turn, game->move_count, game->check);

            // Is the game over?
            if (game->move_count < 0 && game->check)
                {
                    if (!is_game_over_for_player(game->board, game->next_board,
                            game->turn, game->move_count, game->check))
                        {
                ...
                case COMMAND_SELECT:
                    {
                        ...
                        game->num_possible_moves
                            = get_possible_moves_for_piece(game->board,
                                game->selected_piece, game->possible_moves,
...
void
play_chess_networked(ProgramMode mode, ChessGame *game, int connection_fd)
{
    // Play the game of chess!
    view_board(game->board, game->selected_piece, game->num_possible_moves,
        game->possible_moves);
    ...
    while (true)
        {
            ...
            else if (!is_game_stalemate(
                         game->board, game->turn, game->move_count))
                {
                    ...
                    if (game->turn == game->player)
                        {
                            write_network_line(connection_fd,
                                game->input_buffer, (int) game->input_pointer);
                        }
