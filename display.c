...
void view_board(Board board, int selected, size_t possible_moves, PossibleMove possible[MAX_POSSIBLE_MOVES]) {
    print_x_axis();
...
void show_possible_moves(int selected, ChessPiece piece, size_t possible_moves, PossibleMove possible[MAX_POSSIBLE_MOVES]) {
    if (selected < 0) {
        return;
    }
    if (possible_moves < 1) {
    ...
    printf("Possible moves for %s (%c%c):", CHESS_PIECE_STRINGS[piece], INT_TO_COORD(selected));
    for (int i = 0; i < possible_moves; i++) {
        printf(" %c%c (%d) (type: %d)", INT_TO_COORD(possible[i].target), possible[i].target, possible[i].type);
    }
    printf("\n");
}
