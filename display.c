...
void
view_board(Board board, int selected, size_t possible_moves,
                    ...
                    if (!matched)
                        {
                            ...
                            printf(
                                "%s", PIECE_SYMBOLS[piece.colour][piece.type]);
                        }
                    printf("|");
                }
            // Y axis.
...
void
show_prompt(PlayerColour turn, ChessPiece piece)
{
    printf("P%d (%s)", turn + 1, PLAYER_COLOUR_STRINGS[turn]);
    if (piece != PIECE_NONE) printf(" %s", PIECE_SYMBOLS[turn][piece]);
    printf(" %c ", INPUT_PROMPT_SYMBOL);
    fflush(stdin);
}

void
show_promotion_prompt(PlayerColour turn, int selected)
{
...
void
show_question_prompt(const char *question)
{
    printf("%s y/N %c\n", question, INPUT_PROMPT_SYMBOL);
}

void
show_possible_moves(int selected, ChessPiece piece, size_t possible_moves,
    PossibleMove possible[MAX_POSSIBLE_MOVES])
{
