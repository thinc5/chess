...
FileFormat
determine_file_format(ChessGame *game, FILE *file)
{
    ...
    if (strncmp(game->input_buffer, "raw", 3) == 0) { return FORMAT_RAW; }
...
void
replay_chess(ChessGame *game, const char *filepath)
{
    // Does file exist?
    ...
    while (true)
        {
            ...
            // ChessPiece type = game->selected_piece == -1 ? PIECE_NONE :
            ...
            // Read file line by line and throw out any malformed content until
            // eof.
            memset(game->input_buffer, 0, INPUT_BUFFER_SIZE);
            game->input_pointer = 0;
